...
public class SearchCmd extends MusicCommand 
{
    ...
    public void doCommand(CommandEvent event) 
    {
        ...
        {
            event.replyError("请包括查询。");
            return;
        }
        event.reply(searchingEmoji+" 搜寻中。。。 `["+event.getArgs()+"]`",
    ...
    private class ResultHandler implements AudioLoadResultHandler 
    {
        ...
        public void trackLoaded(AudioTrack track)
        {
            ...
            m.editMessage(FormatUtil.filter(event.getClient().getSuccess()+" 已添加 **"+track.getInfo().title
                    +"** (`"+ TimeUtil.formatTime(track.getDuration())+"`) "+(pos==0 ? "准备播放"
                        : " 到队列中的位置 "+pos))).queue();
        }
        ...
        public void playlistLoaded(AudioPlaylist playlist)
        {
            builder.setColor(event.getSelfMember().getColor())
                    .setText(FormatUtil.filter(event.getClient().getSuccess()+" 搜索结果 `"+event.getArgs()+"`:"))
                    ...
                    .setSelection((msg,i) -> 
                    {
                        ...
                        if(bot.getConfig().isTooLong(track))
                        {
                            event.replyWarning("这首歌 （**"+track.getInfo().title+"**） 长于允许的最大值： `"
