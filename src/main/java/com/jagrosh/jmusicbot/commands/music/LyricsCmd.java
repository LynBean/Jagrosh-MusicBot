...
public class LyricsCmd extends MusicCommand
{
    ...
    public void doCommand(CommandEvent event)
    {
        ...
        client.getLyrics(title).thenAccept(lyrics -> 
        {
            if(lyrics == null)
            {
                event.replyError("歌词 `" + title + "` 找不到！" + (event.getArgs().isEmpty() ? " 尝试手动输入歌曲名称 (`lyrics [歌名]`)" : ""));
                return;
            }
            ...
            {
                event.replyWarning("歌词 `" + title + "` 找到但可能不正确： " + lyrics.getURL());
            }
